# Contacts

<form action="https://formspree.io/f/mgebvkwn" method="POST" style="max-width:60ch;margin: 0 auto;"><br><label>Email:<input type="email" name="email" style="width: 100%; background-image: url(&quot;chrome-extension://bmhejbnmpamgfnomlahkonpanlkcfabg/srcdist/assets/passwords-icon-48.png&quot;); background-repeat: no-repeat; background-size: auto 50%; background-position: 674px center; cursor: auto;" autocomplete="off" data-has-passwords-badge="true"></label><br><label>Message:<br><textarea name="message" style="width: 100%;height:10rem"></textarea><br></label><br><button type="submit">Post</button><br></form>

Pour m’écrire tout court :

Thierry Crouzet, 18, rue des Trimarans. 34540 Balaruc les Bains. France.

### Hors du blog

* [Substack](https://tcrouzet.substack.com/) (newsletters, commentaires et chat)
* [Mastodon](https://mamot.fr/@tcrouzet) (mon réseau social)
* [Flipboard](https://flipboard.com/@ThierryCrouzet) (curation)
* [Pixelfed](https://pixelfed.social/p/tcrouzet/) (Instagram libre)
* [GitHub](https://github.com/tcrouzet/) (mes codes et mes textes)
* [Gemini](gemini://gemini.tcrouzet.com) (mirroir low-tech du blog)
* [SourceHut](https://git.sr.ht/~tcrouzet/) (archive au format Gemini)
* [Strava](https://www.strava.com/athletes/18278258) (pour le vélo)
* [Whereby](https://whereby.com/tcrouzet) (visio conf)
* [Wattpad](http://www.wattpad.com/user/ThierryCrouzet) (archives de *One Minute*)
* [YouTube](https://www.youtube.com/user/thierrycrouzet) (peu utilisé)

### Anciens comptes

* Facebook (fermé le 20/3/2025)
* FacebookPage (fermé le 20/3/2025)
* BlueSky (fermé le 20/3/2025)
* Linkedin (fermé le 19/3/2025)
* Instagram (fermé le 19/3/2025)
* Flickr (fermé le 17/3/2025)
* Medium (fermé le 17/3/2025)
* Pinterest (fermé le 17/3/2025)
* tumblr (fermé le 15/3/2025)
* 500px (fermé le 15/32025)
* Twitter (@crouzet fermé le 31/10/2023)

#page #y2013 #2013-1-26-13h17
